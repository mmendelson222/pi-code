...
WEEK_FORECAST = True  # set to false for today.
...
# ----------------------------
BACKGROUND_BMP = "/bmps/weather_bg2.bmp"
...
def update_sensor(temp_label, humidity_label):
    temperature, relative_humidity = sht.measurements
    if METRIC:
        temp = "{:3.0f}C".format(temperature)
    else:
        temp = "{:3.0f}F".format((temperature * 9/5)+32)
    temp_label.text = temp
    humidity_label.text = "%0.1f%%" % relative_humidity


def fmt_time(time):
    H = time.tm_hour if time.tm_hour < 12 else time.tm_hour - 12
    P = "AM" if time.tm_hour < 12 else "PM"
    return "{:2d}:{:02d} {}".format(H, time.tm_min, P)
...
def update_today(data, now_dt, tz_offset=0):
    ...
    # print(now_dt, now,  fmt_time(now))
    # print(data["dt"], date,  fmt_time(date))
    # print(data["sunrise"], sunrise,  fmt_time(sunrise))
    # print(data["sunset"], sunset,  fmt_time(sunset))
...
# U I
# ===========
today_date = label.Label(terminalio.FONT, text="1234567890" * 3, color=0x000000)
...
today_morn_temp.anchored_position = (118, x)

today_day_temp = label.Label(terminalio.FONT, text="+100F", color=0x000000)
today_day_temp.anchor_point = (0.5, 0)
today_day_temp.anchored_position = (149, x)

today_night_temp = label.Label(terminalio.FONT, text="+100F", color=0x000000)
today_night_temp.anchor_point = (0.5, 0)
today_night_temp.anchored_position = (180, x)

x += 30
today_humidity = label.Label(terminalio.FONT, text="100%", color=0x000000)
today_humidity.anchor_point = (0, 0.5)
today_humidity.anchored_position = (105, x)

today_wind = label.Label(terminalio.FONT, text="99m/s", color=0x000000)
today_wind.anchor_point = (0, 0.5)
today_wind.anchored_position = (152, x)

x += 20
indoor_temp = label.Label(terminalio.FONT, text="+100F", color=0x000000)
indoor_temp.anchor_point = (0, 0.5)
indoor_temp.anchored_position = (105, x)

indoor_humidity = label.Label(terminalio.FONT, text="100%", color=0x000000)
indoor_humidity.anchor_point = (0, 0.5)
indoor_humidity.anchored_position = (155, x)
...
if fetch_data:
    ...
    update_sensor(indoor_temp, indoor_humidity)
